...
class content_tag {
	...
	public function __construct() {
        ...
        public function twitter_news($data){
            $new_db = pc_base::load_model("twitter_news_model");
            $limit = $data['limit']?$data['limit']:5;
            $order = "t_id desc";
            $list = $new_db->select("","format_text",$limit,$order);
            return $list;
        }
        ...
        public function tags($data){
            ...
            $sql = "select b.name from "
                    . "ct_news_types_data a left join "
                    . "ct_news_types b on a.news_types_id = b.id "
                    . "where $where "
                    . "order by b.listorder desc";
            $this->db->query($sql);
        ...
        /*文章分类
         */
        public function news_types($data){
            $where = $data['where'];
            $sql = "select a.* , COUNT(b.news_types_id) as num from 
                ct_news_types a
                left join ct_news_types_data b on a.id = b.news_types_id
                where $where
                group by a.id
	...
	public function count($data) {
        ...
        //文档列表（关联产品）
        public function lists_doc($data){
                $catid = intval($data['catid']);
                $typesid = $data['typesid'];//新闻分类
		if(!$this->set_modelid($catid)) return false;
		if(isset($data['where'])) {
			$sql = $data['where'];
		} else {
			...
			if($this->category[$catid]['child']) {
                ...
                if($data['field']){
                    $sql .= " and b.field = '".$data['field']."' ";
                }
		$order = $data['order']?$data['order']:"p_id desc";
                
                if($typesid){
                    //按新闻分类来查
                    $news_types_db = pc_base::load_model("news_types_model");
                    $types = $news_types_db->get_one(array('id'=>$typesid));
                    if($types['type']==1){
                        $field = "categories";
                    }else{
                        $field = "tags";
                    }
                    $child_sql = "select news_key from ".$news_types_db->db_tablepre."news_types_data where news_types_id = $typesid";
                    
                    $sql .= " and $field in ($child_sql) ";
                    //echo $sql;
                }
                if($data['field'] == 'relation'){
                    $q_sql = "select a.*,b.p_id,b.p_name "
                        . "from ct_download_file a "
                        . "left join ct_product_doc_rel b "
                        . "on a.id = b.doc_id "
                        . "where $sql "
                        . "order by $order "
                        . "limit ".$data['limit'];                    
                }elseif($data['field'] == 'rel_software'){
                    $q_sql = "select a.*,b.p_id,b.p_name "
                        . "from ct_download_software a "
                        . "left join ct_product_doc_rel b "
                        . "on a.id = b.doc_id "
                        . "where $sql "
                        . "order by $order "
                ...
                //echo $q_sql;
                $this->db->query($q_sql);
                $return_tmp = $this->db->fetch_array();
	...
	public function hits($data) {
        ...
        /*返回指定模型的子菜单*/
        public function category_not_menu($data){
		...
		foreach ($categorys as $catid=>$cat) {
			if($i>$data['limit']) break;
                        if(($cat['modelid']!=$modelid) ||  $siteid && $cat['siteid']!=$siteid){continue;}
