	...
	function select_list(obj,title,id) {
		...
		if($(obj).attr('class')=='line_ff9966' || $(obj).attr('class')==null) {
			...
			if(relation_ids !='' ) {
				...
				window.top.$('#<?php echo $field;?>').val(newrelation_ids);
			}
		} else {
			$(obj).attr('class','line_ff9966');
			var str = "<li id='<?php echo $field;?>"+sid+"'>·<span>"+title+"</span><a href='javascript:;' class='close' onclick=\"remove_relation('<?php echo $field;?>"+sid+"',"+id+")\"></a></li>";
			window.top.$('#<?php echo $field;?>_text').append(str);
			if(relation_ids =='' ) {
				window.top.$('#<?php echo $field;?>').val(id);
			} else {
				relation_ids = relation_ids+'|'+id;
				window.top.$('#<?php echo $field;?>').val(relation_ids);
			}
		}
}
//-->
