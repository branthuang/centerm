...
//定义在单独操作内容的时候，public_getjson_ids同时更新相关栏目页面
...
class content extends admin {
		...
		if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
			...
			if($category['type']==0) {
				...
				$id = $this->db->add_content($_POST['info']);
                                if($_POST['info']['relation'] || $_POST['info']['rel_software']){
                                    $rel_db = pc_base::load_model("product_doc_rel_model");
                                    $where = array('p_id'=>$id);
                                    $rel_db->delete($where);
                                    if($_POST['info']['relation']){
                                        //产品和文档的关联单独做一张表保存
                                        $relation = $_POST['info']['relation'];
                                        $rel_arr = explode('|', $relation);
                                        $rel_arr = array_unique($rel_arr);                                    
                                        foreach ($rel_arr as $rel){
                                            $data = array(
                                                'p_id' => $id,
                                                'p_name' => $_POST['info']['title'],
                                                'doc_id' => $rel,
                                                'field' => 'relation'
                                            );
                                            $rel_db->insert($data);
                                    ...
                                    if($_POST['info']['rel_software']){
                                        //产品和软件的关联单独做一张表保存
                                        $rel_software = $_POST['info']['rel_software'];
                                        $rel_arr = explode('|', $rel_software);
                                        $rel_arr = array_unique($rel_arr);
                                        foreach ($rel_arr as $rel){
                                            $data = array(
                                                'p_id' => $id,
				...
				$_POST['info']['style'] = strip_tags($_POST['style_color']).';'.$style_font_weight;
                                if(!empty($_POST['images_url'])){
                                    //多图片字段处理
                                    foreach($_POST['images_url'] as $k=>$val){
                                        $images[$k] =  array(
                                            'url' => $val,
                                            'alt' => $_POST['images_alt'][$k]
                                        );
                                    }
                                    $images = new_htmlentities(array2string($images));
                                    $_POST['info']['images'] = $images;
                                }
				if($_POST['edit']) {
	...
	public function public_relationlist() {
        ...
        /*关联到其他模型的文章
         */
        public function public_getjson_ids_other(){
                $rel_modelid = intval($_GET['rel_modelid']); //关联模型
                $modelid = intval($_GET['modelid']);
		$id = intval($_GET['id']);
                $field = $_GET['field']?$_GET['field']:'relation';//保存关联关系的字段名
                
		$this->db->set_model($modelid);
		$tablename = $this->db->table_name;
		$this->db->table_name = $tablename.'_data';
		$r = $this->db->get_one(array('id'=>$id),$field);
