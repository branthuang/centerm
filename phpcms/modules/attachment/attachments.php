...
class attachments {
	...
	public function upload() {
		...
		$result = $att_db->get_one($where);
                $filename = $result['filename']; //文件名
                
                pc_base::load_sys_class('attachment','',0);
                $attachment = new attachment();
                $attachment->delete($where); //删除记录以及文件
                $result = array(
                    $filename => true
                );
        ...
        public function jfupload(){
            ...
            if(isset($_FILES['files'])){
                    ...
                    $watermark_enable = $_POST['watermark_enable']; //添加水印
                    ...
                    $site_setting = get_site_setting($siteid);
                    $site_allowext = $site_setting['upload_allowext'];
                    $aids = $attachment->upload('files',$site_allowext,'','',array($thumb_width,$thumb_height),$watermark_enable);
                    if($aids[0]) {
                            ...
                            if($attachment->uploadedfiles[0]['isimage']) {
                                    //$fileext = $attachment->uploadedfiles[0]['isimage'];
                            ...
                            $deleteUrl = buildurl('del_file','attachments','attachment',array('aid'=>$aids[0]));
                            $result = array(
                                'files'=>array(
                                        '0' =>array(
                                            'name' => $filename,
                                            'size' => $attachment->uploadedfiles[0]['filesize'],
                                            'type'=> $fileext,
                                            "url" => $url,
                                            'aid' => $aids[0],
                                            'thumbnailUrl' => $fileext=='jpg'?$this->upload_url.$attachment->uploadedfiles[0]['filepath']:'', //图片才有缩略图
                                            'deleteUrl' => $deleteUrl,
                                            'deleteType' => 'DELETE'
                                    )
                                )
                    ...
                    $watermark_enable = $this->site_setting['watermark_enable'];

                    include $this->admin_tpl('jfupload');
            }
        }
	/**
	 * swfupload上传附件
	...
	public function swfupload(){
            $this->jfupload();//swf上传用jquery上传库替换
            exit;
		$grouplist = getcache('grouplist','member');
