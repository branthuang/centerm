...
class form {
	...
	/*
         * 多图片上传
         */
        public static function multy_images($field, $value, $fieldinfo,$catid) {
		extract($fieldinfo);
		$list_str = '';
		if($value) {
		...
		$string .= $str."<div class='picBut cu'><a href='javascript:void(0);' onclick=\"javascript:flashupload('{$field}_images', '".L('attachment_upload')."','{$field}',change_images,'{$upload_number},{$upload_allowext},{$isselectimage}','content','$catid','{$authkey}')\"/> ".L('select_pic')." </a></div>";
		return $string;
	}
	/**
	 * 
	...
	public static function select_category($file = '',$catid = 0, $str = '', $default_option = '', $modelid = 0, $type = -1, $onlysub = 0,$siteid = 0,$is_push = 0) {
        ...
        //选择框，只显示联动菜单一级，可以添加事件
        public static function select_with_func($keyid, $name,$id='',$alt='',$return_func='',$property='',$default=''){
            
            $result = getcache($keyid,'linkage');
            $id = $id ? $id : $name;
            $onchange = '';
            if($return_func){
                $onchange = "onchange = \"$return_func(this.options[this.options.selectedIndex].value)\"";
            }
            $string = "<select name='$name' id='$id' $property $onchange>\n<option value='0'>$alt</option>\n";
            if($result['data']) {
                    foreach($result['data'] as $area) {	
                        if($area['parentid'] == 0){
                            //一级
                            $selected = '';
                            if($default == $area['linkageid']){
                                $selected = 'selected';
                            }
                            $string .= 	"<option value='".$area['linkageid']."' $selected>".$area['name']."</option>";
                        }  
                    }
            }
