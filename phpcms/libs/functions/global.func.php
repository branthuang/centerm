...
/*前台自定义分页函数*/
function ct_pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	if(defined('URLRULE') && $urlrule == '') {
		$urlrule = URLRULE;
		$array = $GLOBALS['URL_ARRAY'];
	} elseif($urlrule == '') {
	...
	if($num > $perpage) {
		...
		//$multipage .= '<a class="a1">'.$num.L('page_item').'</a>';
		if($curr_page>0) {
			$multipage .= ' <li><a href="'.pageurl($urlrule, $curr_page-1, $array).'" >&laquo;</a></li>';
			if($curr_page==1) {
		...
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= ' ..<li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li> '
                                        . '<li><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">&raquo;</a></li>';
			} else {
				$multipage .= ' <li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li> '
                                        . '<li><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">&raquo;</a></li>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= ' <li class="active"><a href="###">'.$pages.'</a></li> <li><a href="'.pageurl($urlrule, $curr_page, $array).'">&raquo;</a></li>';
		} else {
			$multipage .= ' <li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li> '
                                . '<li><a href="'.pageurl($urlrule, $curr_page+1, $array).'">&raquo;</a></li>';
		}
	}
	return $multipage;
}
...
function catpos($catid, $symbol=' > '){
	...
	foreach($arrparentid as $catid) {
		...
		$pos .= '<li><a href="'.$url.'">'.$category_arr[$catid]['catname'].'</a></li>';
	}
	return $pos;
}
