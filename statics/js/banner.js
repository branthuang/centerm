...
window.onload=function(){
	if(document.getElementById('focus_m')){
		focusRun.init();		
	}
	if(document.getElementById('marquee')){
		marquee();
	}
	if(document.getElementById('piclist_m')){
		//存在该元素时，开始事件
		focusRun2.init();
	}
};
...
piclist.prototype={
	init:function(){
		...
		this.sTime=this.sTime || 5000;
		//图片容器
		this.oImg=document.getElementById('piclist_m').getElementsByTagName("ul")[0];
		this.oImgLi=this.oImg.getElementsByTagName("li");
		//左右按钮
		this.oL=document.getElementById('piclist_l');
		this.oR=document.getElementById('piclist_r');
		//创建文字容器
		this.createTextDom();
		//默认第一帧
		this.target=0;
	...
	createTextDom:function(){
		...
		//创建文字容器
		this.oText=document.createElement("div");
		this.oText.className="piclist_s";
		var ul=document.createElement('ul');
		var frag=document.createDocumentFragment();
		for (var i=0;i<this.oImgLi.length;i++) {
			var li=document.createElement("li");
...
var focusRun2=new piclist();
focusRun2.o=document.getElementById("piclist");
function mouseEnter(ele,type,func){
	if(window.document.all)	
		ele.attachEvent('on'+type,func);
	else{//ff
