...
//移除相关文章
function remove_relation(sid,id,field) {
	var relation_ids = $('#'+field).val();
	if(relation_ids !='' ) {
		$('#'+sid).remove();
		...
		$('#'+field).val(newrelation_ids);
	}
}

/*显示相关文章，非同一模型
 * 
 * @param {type} rel_modelid 关联模型
 * @param {type} modelid 本文章模型
 * @param {type} id 本文章id
 * @param {type} field 本文章关联字段
 * @param {type} back_id js返回id
 * @returns {undefined}
 */
function show_relation_other(rel_modelid, modelid, id, field ){
    $.getJSON("?m=content&c=content&a=public_getjson_ids_other&rel_modelid="+rel_modelid+"&modelid="+modelid+"&id="+id+"&field="+field, function(json){
	var newrelation_ids = '';
	if(json==null) {
		alert('没有添加相关内容');
		return false;
	...
	$.each(json, function(i, n){
           	newrelation_ids += "<li id='"+field+n.sid+"'>·<span>"+n.title+"</span><a href='javascript:;' class='close' onclick=\"remove_relation('"+field+n.sid+"',"+n.id+",'"+field+"')\"></a></li>";
	});

        $('#'+field+'_text').html(newrelation_ids);
    }); 
}
//显示相关文章
